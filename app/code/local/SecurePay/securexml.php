...
/* Fraud Guard */
     ...
     * __construct.
     *
     * @param int    $gatewaymode      One of SECUREPAY_GATEWAY_MODE*
...
/* @param string/int year YY or year Y or year YYYY - If there are leading zeros, type needs to be string*/
    ...
    {
        if (!$this->getTxnType()) {
            //i.e. If not already set to FraudGuard
...
/* This indicates to the XML constructor that payment details should be included in the request. */
     ...
     * 
     * @return bool Returns true for succesful (approved) transaction / false for failure (declined) or error
     */
...
//The transaction is successful and "approved" if the above tests have been passed
     ...
     * 
     * @return bool Return TRUE for all checks passed OK, or FALSE if an error is detected
     */
    private function checkCCParameters()
    {
        //$ccNumber must be numeric, and have between 12 and 19 digits
        if (strlen($this->getCCNumber()) < 12 || strlen($this->getCCNumber()) > 19 || preg_match("/\D/", $this->getCCNumber())) {
            //Regex matches non-digit
...
//CVV is optional
    ...
    private function checkTxnParameters()
    {
        ...
        if ($this->getRequestType() == SECUREPAY_REQUEST_PAYMENT) {
            ...
            if ($this->getTxnType() == SECUREPAY_TXN_DIRECTDEBIT || $this->getTxnType() == SECUREPAY_TXN_DIRECTCREDIT) {
                ...
                if (strlen($ref) == 0 || strlen($ref) > 18 || preg_match('/[^0-9a-zA-Z*\.&\/-_\']/', $ref)) {
                    //Regex matches non-EBCDIC characters
                ...
                if (strlen($ref) == 0 || strlen($ref) > 60 || preg_match('/[ \']/', $ref)) {
                    //Regex matches space or '
    ...
    private function createXMLTransactionRequestString()
    {
        ...
        if ($requestType == SECUREPAY_REQUEST_PAYMENT) {
            ...
            $item = 'Txn';
            $apiVer = 'xml-4.2';
        } elseif ($requestType == SECUREPAY_REQUEST_PERIODIC) {
            $list = 'PeriodicList';
            $item = 'PeriodicItem';
            $apiVer = 'spxml-4.2';
        } else {
        ...
        if ($requestType != SECUREPAY_REQUEST_ECHO) {
            ...
            if ($requestType == SECUREPAY_REQUEST_PERIODIC) {
                ...
                } elseif ($this->getAction() == SECUREPAY_ACTION_TRIGGER) {
                    if ($this->getAmount()) {
                        $x .=    '<amount>'.htmlentities($this->getAmount()).'</amount>';
                    }
                }
            } elseif ($requestType == SECUREPAY_REQUEST_PAYMENT) {
...
//Trim headers
     ...
     *
     * @return bool True to indicate succesful decoding of response message AND succesful txn result, false to indicate an error or declined result
     */
...
/* Common response values */
        ...
        } elseif (strtoupper($responseArray['approved']) == 'YES' && ($this->getTxnType() == SECUREPAY_TXN_DIRECTDEBIT || $this->getTxnType() == SECUREPAY_TXN_DIRECTCREDIT)) {
...
/* field 'successful' = 'Yes' means 'triggered transaction successfully registered', anything else is failure */
     ...
     *
     * @return bool True to indicate succesful conversion of document, false to indicate an error 
     */
...
case 'complete':
                    ...
                    if (array_key_exists('value', $val)) {
                        ...
                        // to handle empty self closing tags i.e. <paymentInterval/>
